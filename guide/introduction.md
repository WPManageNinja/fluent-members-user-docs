...
Fluent Members doesn't process payments on its own. It works with your existing payment plugin:

| Payment plugin | What it handles |
|---|---|
| **FluentCart** | Native WPManageNinja checkout and subscriptions |
| **WooCommerce** | Products, variations, WooCommerce Subscriptions |
| **Fluent Forms** | Payment forms with fixed-price plans |
| **Paymattic** | Payment forms with fixed-price plans |

You need at least one of these installed and configured before you can sell memberships. If you're starting fresh, **FluentCart** is the smoothest fit — it's from the same family.

Fluent Members also connects to:
- **FluentCRM** — automate email sequences when a member joins, upgrades, is suspended, or expires
- **Fluent Support** — show the support team which memberships each customer holds

::: info No payment plugin yet?
That's okay — you can still install Fluent Members, create your Levels and Access Groups, and add a payment plugin later. Nothing breaks. You just won't be able to sell until a payment provider is connected.
:::
...
| Payment plugin | FluentCart, WooCommerce, Fluent Forms, or Paymattic |
...
**Related reading:**
- [Core Concepts explained](./core-concepts/index.md) — a deeper look at Levels, Groups, and Members
